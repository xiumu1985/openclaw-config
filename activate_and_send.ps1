# Activate Feishu and send the specific message
...
"@
    ...
    if ($feishuProcess.MainWindowHandle -ne [IntPtr]::Zero) {
        ...
        # Alternative: try to start the application again
        $existingProc = Get-Process -Name "Feishu" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($existingProc) {
            [NativeMethods]::SetForegroundWindow($existingProc.MainWindowHandle)
        }
    }
    ...
    Write-Output "Successfully attempted to send '你在干嘛呢' to '朱宝'"
